...
echo "Running professional design thinking in public policy companion workflows..."
...
if command -v python3 >/dev/null 2>&1; then
  ...
  python3 public_policy_design_engine.py \
...
if command -v sqlite3 >/dev/null 2>&1; then
  ...
  sqlite3 outputs/public_policy_design.db < sql/analytical_queries.sql > outputs/sql_analysis_results.txt
else
